...
gg_sc_det_comparison <- ggplot(data = sc_det_df, 
      ...
      scale_y_log10("Standard deviation [-]",
                    ...
                    breaks=c(min(sc_det_df$sd, na.rm=T), 1)) +
...
gg_sc_stoc_comparison <- ggplot(data = sc_stoc_df, 
      ...
      scale_y_log10("Standard deviation [-]",
                    labels = trans_format("log10", math_format(10 ^ .x)),
                    breaks=c(min(sc_stoc_df$sd, na.rm=T), 1)) +
      scale_x_log10("Aggregation scale [-]",
                    labels = trans_format("log10", math_format(10 ^ .x))) +
      scale_fill_manual("", values = cols) +
      scale_colour_manual("", values = cols) +
...
scale_montreal_ncep_6h <- data.table(montreal_ncep[, scalegram(value, plot = F), variable]) #Waiting for chris to fix this
...
#ncep_raw_matrix <- dcast(data = montreal, time~variable, value.var = "value")   #Transformation for parallel computing
#empirical_scalegrams <- scalegram_parallel(ncep_raw_matrix[-1])

scale_montreal_ncep_month <- data.table(montreal_ncep_month[, scalegram(value, plot = F), variable]) #Waiting for chris to fix this
```

> *Discussion: Is the above script in line with the workflow principles of our project?*  

**Comparing different scalegrams**
...
dummy <- sc_montreal_6h[variable == "precipitation"]
...
empirical_scalegram_precip_rescaled <- rescale_variance( #Var. of the 6h scalegrams in the monthly scale  
  ...
  emp_scalegram_fine = sc_montreal_6h[variable == "precipitation"],
  scale_ratio = 4 * 30)
...
*Exercise 3: Plot in one scalegram 6-h, daily, monthly and annual scalegrams.*

*Exercise 4: Compare scalegrams of T and P between Montreal and another location.* 
...
``` r
dummy_1 <- sc_montreal_6h[variable == "precipitation"]
