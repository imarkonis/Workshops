  ...
  if("variable" %in% colnames(X)){
    ggplot(data=X, aes(x=scale, y=var_scale))+
      geom_line(aes(group=interaction(variable),
                    colour = variable, alpha = 1/as.numeric(variable)), show_guide = FALSE, size = 0.5) +
      geom_point(aes(group=interaction(variable),
                     colour=variable, alpha = 1/as.numeric(variable)), show_guide = FALSE) +
    ...
    ggplot(data=X, aes(x=scale, y=var_scale))+
      geom_line(show_guide = FALSE, size = 0.5) +
